 ...
 */
public class MultSlidingWIndowRateLimit {
  ...
  public MultSlidingWIndowRateLimit(Vertx vertx, RedisClient redisClient, Future<Void> completed) {
    this.redisClient = redisClient;
    vertx.fileSystem().readFile("multi_sliding_window_ratelimit.lua", res -> {
      if (res.failed()) {
      ...
      redisClient.scriptLoad(res.result().toString(), ar -> {
        if (ar.succeeded()) {
          luaScript = ar.result();
          LOGGER.info("load multi_sliding_window_ratelimit.lua succeeded");
          completed.complete();
        } else {
          ar.cause().printStackTrace();
          LOGGER.error("load multi_sliding_window_ratelimit.lua failed", ar.cause());
  ...
  public void rateLimit(List<SlidingWindowRateLimitOptions> limits, Handler<AsyncResult<RateLimitResponse>> handler) {
    if (luaScript == null) {
      handler.handle(Future.failedFuture("multi_sliding_window_ratelimit.lua is not loaded yet"));
      return;
    }
    ...
    redisClient.evalsha(luaScript, keys, args, ar -> {
      if (ar.failed()) {
        ar.cause().printStackTrace();
        LOGGER.error("eval multi_sliding_window_ratelimit failed", ar.cause());
